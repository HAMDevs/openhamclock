...
import { filterDXPaths, detectMode } from "../utils";
...
export const WorldMap = ({
      ...
      filteredPaths.forEach((path) => {
        try {
          ...
          const dxCircle = L.circleMarker([dxLatDisplay, dxLonDisplay], {
            ...
            .on('click', () => {
              if (path.freq) {
                // Heuristic: if freq is small (< 100), it's MHz, else Hz.
                // HamClock generally provides MHz string.
                const freqHz = parseFloat(path.freq) * 1000000;
                tuneTo(freqHz);
              }
            })
            .addTo(map);
  ...
  }, [dxPaths, dxFilters, showDXPaths, showDXLabels, hoveredSpot, tuneTo]);
      ...
      potaSpots.forEach(spot => {
        if (spot.lat && spot.lon) {
          ...
          const marker = L.marker([spot.lat, spot.lon], { icon: triangleIcon })
            ...
            .on('click', () => {
              // POTA spots usually act as MHz string? Need to verify format.
              // Assuming MHz based on context.
              const freqVal = parseFloat(spot.freq);
              if (!isNaN(freqVal)) {
                // Check if it's already in Hz (large number) or MHz
                const freqHz = freqVal > 100000 ? freqVal : freqVal * 1000; // POTA often uses kHz? or MHz?
                // Standardizing: if < 100 it is MHz. If > 100000 it is Hz.
                // Actually POTA API often returns kHz.
                // Let's assume kHz if > 1000 and < 100000000
                // But safer to assume input spot.freq is consistent with app usage.
                // HamClock POTA hook usually formats freq.
                // Let's check POTAPanel usage later, but for now duplicate logic
                let targetHz = freqVal;
                if (freqVal < 1000) targetHz = freqVal * 1000000; // MHz -> Hz
                else if (freqVal < 100000) targetHz = freqVal * 1000; // kHz -> Hz

                const mode = spot.mode || detectMode(spot.locationDesc || spot.comment || '');
                tuneTo(targetHz, mode);
              }
            })
            .addTo(map);
  ...
  }, [potaSpots, showPOTA, showDXLabels, tuneTo]);
        ...
        if (!isNaN(spotLat) && !isNaN(spotLon)) {
          ...
          try {
            ...
            `)
              .on('click', () => {
                if (freqMHz && freqMHz !== '?') {
                  tuneTo(parseFloat(freqMHz) * 1000000, spot.mode);
                }
              })
              .addTo(map);
  ...
  }, [pskReporterSpots, showPSKReporter, deLocation, tuneTo]);
  ...
  }, [wsjtxSpots, showWSJTX, deLocation, tuneTo]);
