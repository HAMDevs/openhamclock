...
app.get('/api/satellites/tle', async (req, res) => {
  try {
    ...
    // Parallel fetch: TLEs from CelesTrak AND Frequencies from Palewire
    const groups = ['amateur', 'weather', 'goes'];
    logDebug("[Satellites] Fetching TLEs and Frequency data...");

    const [freqResponse, ...tleResponses] = await Promise.all([
      // 1. Fetch Frequency Data (Palewire)
      fetch(
        ...
        .catch((e) => {
          logWarn("[Satellites] Failed to fetch frequency DB:", e.message);
          return [];
        }),

      // 2. Fetch TLE Data (CelesTrak - Multiple Groups)
      ...groups.map((group) =>
        fetch(
          ...
          .catch((e) => {
            logWarn(
              `[Satellites] Failed to fetch TLE group ${group}:`,
              e.message,
            );
            return "";
          }),
    ...
    if (Array.isArray(freqResponse)) {
      freqResponse.forEach((sat) => {
        if (sat.norad_id) {
          freqMap.set(parseInt(sat.norad_id), sat);
        }
      });
      logDebug(`[Satellites] Loaded ${freqMap.size} frequency records`);
    }

    // Process TLEs
    tleResponses.forEach((text, index) => {
      ...
      for (let i = 0; i < lines.length - 2; i += 3) {
        ...
        if (name && line1 && line1.startsWith("1 ")) {
          ...
          // Merge frequency data if available
          const freqData = freqMap.get(noradId) || {};

          // Construct transmitters array from flat Palewire fields if 'transmitters' doesn't exist
          let transmitters = freqData.transmitters || [];
          if (transmitters.length === 0) {
            if (freqData.downlink) {
              transmitters.push({ description: "Downlink", frequency: freqData.downlink, mode: freqData.mode });
            }
            if (freqData.uplink) {
              transmitters.push({ description: "Uplink", frequency: freqData.uplink, mode: freqData.mode });
            }
            if (freqData.beacon) {
              transmitters.push({ description: "Beacon", frequency: freqData.beacon, mode: freqData.mode });
            }
          }
